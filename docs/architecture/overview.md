...
2. **Real Agent Execution Layer**: OpenClaw / Transpect runs the real agent and produces the canonical `monitor/live/runs/<runId>/` directory.
...
The current security implementation is online and fused into the runtime through `guardrail/agent_defense/` plus the OpenClaw behavior mediator. It supports the staged Xiaohongshu watering-hole demo by maintaining security state fields such as intent constraints, source trust chain, navigation chain, scope deviation, action risk, sensitive resources, and evidence events while the agent runs. It emits `security-reasoning/security_state.json`, `security-reasoning/defense_decision.json`, `security-reasoning/evidence_summary.json`, and `security-reasoning/final_judgment.json`.

`tools/security_reasoning/run_defense_reasoner.py` is retained for compatibility with the earlier contextual reasoner. `tools/security_context/run_context_judge.py` remains as a compatibility wrapper that writes the older `security-context/` reports.
...
- OTEL: `monitor/vendor/external/openclaw-observability-plugin/` writes optional outputs under `monitor/live/otel/`.
- Frida: `tools/capture/capture_frida.py` writes optional host-side capture output under `monitor/live/frida/`.

These support diagnosis, but they do not replace `monitor/live/runs/<runId>/` as the primary runtime record.

## Script Organization

The repository groups scripts by responsibility:

- `tools/common/` for shared path and utility helpers
- `tools/runtime/` for runtime setup, startup, cleanup, and viewer serving
- `tools/export/` for CodeTracer bundle generation
- `tools/diagnosis/` for diagnosis execution and legacy segmentation
- `tools/security_reasoning/` for contextual defense state and decisions
- `tools/security_context/` for compatibility security context reports
- `tools/validate/` for repo, topology, and acceptance checks
- `tools/capture/` for optional capture tooling
- `tools/compat/` for legacy wrapper support

Flat entrypoints in `tools/*.py` remain as compatibility wrappers only.

## Legacy Notes

- `monitor/live/behavior-events.jsonl` is legacy migration input only.
- `harvest/` is not part of the current architecture.
- `monitor/live/archive/` remains available for optional archival and migration workflows.
