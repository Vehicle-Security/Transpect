...
```
┌─────────────────────────────────────────────────────────────┐
│ Layer 1: Task Source          task_repos/                   │
...
```
app/agent_defense/  ──imports──>  app/security/
    协调层                          能力层
    (bridge/engine/policy)         (guards/risk/decision)

app/security/ 绝不 import app/agent_defense/
```

| 层 | 包 | 职责 | 对外接口 |
|---|---|---|---|
| 协调层 | `app/agent_defense/` | bridge 入口、策略引擎、bypass 检测、动作规范化、trace 合并、最终判定 | `handle()` |
| 能力层 | `app/security/` | intent/plan/action 守卫、风险评分、信任模型、命令策略、决策引擎、LLM 灰区裁判 | `inspect_*` 函数 |
| 插桩 | `app/instrumentation/frida/` | 可选 Frida 运行时追踪（观察性，优雅降级） | `FridaManager` |
| 场景 | `app/runtime/agent_scenarios/` | OpenClaw 客户端和场景化报告构建 | — |
...
# 仓库健康检查
python scripts/validate/check_repo.py --skip-start  # PASS
