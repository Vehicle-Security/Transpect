...
from app.instrumentation.frida.frida_resolver import FridaResolution, FridaResolver
...
class TestFridaResolver(unittest.TestCase):
    ...
    @patch("app.instrumentation.frida.frida_resolver.inspect.getfile")
    @patch("shutil.which")
    def test_frida_shadowed_by_local_dir(self, mock_which: MagicMock, mock_getfile: MagicMock) -> None:
        mock_which.return_value = "/usr/local/bin/frida-ps"
    ...
    def test_frida_properly_installed_but_cli_missing(self, mock_which: MagicMock, mock_getfile: MagicMock) -> None:
        ...
        with patch.dict("sys.modules", {"frida": frida_mock}):
            with patch("app.instrumentation.frida.frida_resolver.sys.executable", "/tmp/no-frida-env/bin/python"):
                res = self.resolver.resolve()
            self.assertTrue(res.import_ok)
    ...
    @patch("app.instrumentation.frida.frida_resolver.FridaResolver._system_arch")
    @patch("shutil.which")
