...
class AgentTraceRunnerTests(unittest.TestCase):
    ...
    def test_mode_agent_trace_with_mock_openclaw(self) -> None:
        ...
        with patch("run_task_repo.run_openclaw_agent", return_value={"ok": True, "runId": "run-123"}), patch(
            ...
            "run_task_repo.run_context_judge",
            return_value={
                "ok": True,
                "decision": "warn",
                "riskLevel": "medium",
                "score": 55,
                "paths": {"report": "security-context/context_report.json"},
            },
        ) as context_judge, patch("run_task_repo.create_task_repo_run") as create_task_repo_run:
            report = self.run_main(
        ...
        self.assertTrue(context_judge.called)
        self.assertEqual(report["securityContext"]["decision"], "warn")
