 ...
 * See vendor/openclaw-observability-plugin/instrumentation/preload.mjs.
 */
...
export async function initOpenLLMetry(_config: OtelObservabilityConfig, logger: any): Promise<void> {
  ...
  if (preloadActive) {
    ...
    logger.info(
      "[otel]   To enable: set NODE_OPTIONS='--import vendor/openclaw-observability-plugin/instrumentation/preload.mjs' before starting openclaw"
    );
