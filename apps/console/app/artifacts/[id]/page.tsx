  ...
  return (
    <AppShell>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <Link href={`/showcases/${id}`} className="action-button-light">
          Back to Report
        </Link>
      </div>
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        <aside className="panel max-h-[70vh] overflow-hidden p-4">
          ...
          <div className="mt-4 max-h-[calc(70vh-68px)] space-y-2 overflow-y-auto pr-1">
            {artifacts.map((artifact) => (
              <Link
                key={artifact.path}
                href={`/artifacts/${id}?path=${encodeURIComponent(artifact.path)}`}
                className={`block rounded-md border p-3 text-sm transition ${selected === artifact.path ? "border-slate-900 bg-slate-100" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`}
              >
                <div className="flex min-w-0 items-center justify-between gap-2">
                  <span className="min-w-0 truncate font-semibold text-slate-950">{artifact.name}</span>
                  <Badge value={artifact.status} />
                </div>
                <p className="mono mt-2 truncate text-xs leading-5 text-slate-500" title={artifact.path}>
                  {artifact.path}
                </p>
              </Link>
            ))}
        ...
        <section className="panel self-start overflow-hidden">
          <div className="border-b border-slate-200 p-4">
            <h2 className="mono truncate text-sm font-semibold text-slate-950" title={selected || undefined}>{selected || "No artifact selected"}</h2>
            {content?.truncated ? <p className="mt-1 text-xs text-amber-700">Large file truncated for browser display.</p> : null}
          </div>
          <pre className="mono max-h-[70vh] overflow-auto bg-slate-950 p-5 text-[13px] leading-6 text-slate-100">
