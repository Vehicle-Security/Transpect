...
CONSOLE_DIR = REPO_ROOT / "dashboard" / "console"
SHOWCASE_ROOT = REPO_ROOT / "dashboard" / "state" / "showcase"
...
def check_console_app() -> list[str]:
    ...
    if not (CONSOLE_DIR / "node_modules").exists():
        issues.append("Missing dashboard/console/node_modules. Run: cd dashboard/console && npm ci")
    return issues
...
def check_showcase_reports() -> tuple[list[str], int]:
    ...
    if not isinstance(showcases, list) or not showcases:
        return ["dashboard/state/showcase/index.json does not list any showcases."], 0
...
def main(argv: list[str] | None = None) -> int:
        ...
        if not args.check_only:
            ...
            if status == 200:
                print(
                    ...
                    "Restart with: python tools/demo/start_console.py --port "
                    f"{args.port} --restart",
                    file=sys.stderr,
                )
                return 3
    ...
    if args.check_only:
        print(f"\nConsole is ready to start: cd dashboard/console && npm run dev -- --hostname {args.host} --port {args.port}")
        return 0
