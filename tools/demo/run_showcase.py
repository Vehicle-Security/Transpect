...
sys.path.insert(0, str(ROOT / "tools" / "diagnosis"))
sys.path.insert(0, str(ROOT / "tools" / "security_reasoning"))
sys.path.insert(0, str(ROOT / "tools" / "export"))
sys.path.insert(0, str(ROOT))

from guardrail.agent_defense.final_judge import run_final_judgment  # noqa: E402
from guardrail.agent_defense.trace_merge import merge_run_traces  # noqa: E402
from monitor.trace_model.build_canonical_trace import build_canonical_trace  # noqa: E402
from tools.export.export_openinference_trace import export_openinference_trace  # noqa: E402
from tools.validate.evaluate_trace_quality import evaluate_trace_quality  # noqa: E402
...
def ensure_demo_site(*, host: str = "127.0.0.1", port: int = 8765, auto_start: bool = True) -> dict[str, Any]:
    ...
    _start_background(
        "staged-attack-site",
        [python_executable(), str(ROOT / "tools" / "demo" / "run_staged_attack_site.py"), "--host", host, "--port", str(port)],
    )
    for _ in range(20):
        if _probe(f"http://{host}:{port}/health"):
            return {"status": "ok", "detail": url}
...
def ensure_viewer(*, host: str = "127.0.0.1", port: int = 8711, auto_start: bool = True) -> dict[str, Any]:
    ...
    _start_background(
        "viewer",
        [python_executable(), str(ROOT / "tools" / "runtime" / "serve_viewer.py"), "--host", host, "--port", str(port)],
    )
    for _ in range(20):
        if _probe(f"{base}/health"):
            return {"status": "ok", "detail": base}
...
def check_frida_smoke(*, timeout_seconds: int = 75, verbose: bool = False) -> dict[str, Any]:
    ...
    command = [
        python_executable(),
        str(ROOT / "tools" / "validate" / "frida_smoke.py"),
        "--report",
        str(report_path),
    ]
    try:
...
def run_agent_trace(*, task_id: str = DEFAULT_TASK_ID, timeout_seconds: int, verbose: bool) -> Path:
    command = [
        python_executable(),
        str(ROOT / "tools" / "runtime" / "run_task_repo.py"),
        "--repo",
        "staged_attack",
        "--mode",
        "agent-trace",
...
def complete_artifacts(run_dir: Path, *, verbose: bool) -> dict[str, Any]:
    ...
    mark_showcase_run(run_dir, reason="Generated by tools/demo/run_showcase.py")
