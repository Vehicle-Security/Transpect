...
def resolve_viewer_workspace_url(url: str | None) -> Path | None:
    if not url:
        return None
    normalized_url = posixpath.normpath(posixpath.join("/viewer", url.split("?", 1)[0].split("#", 1)[0]))
    if normalized_url == "/live" or normalized_url.startswith("/live/"):
        return (TRACE_LIVE_DIR / normalized_url.removeprefix("/live").lstrip("/")).resolve()
    return (TRACE_ROOT / normalized_url.lstrip("/")).resolve()


def read_live_url() -> str | None:
    shared_js = TRACE_VIEWER_DIR / "shared.js"
...
def current_paths(config: dict[str, Any]) -> dict[str, Any]:
    ...
    resolved_live_path = resolve_viewer_workspace_url(live_url)
    runs_index = build_runs_index_payload(TRACE_LIVE_RUNS_DIR)
    viewer_path = (TRACE_VIEWER_DIR / "index.html").resolve()
    return {
        ...
        "viewer": {
            ...
            "liveUrl": live_url,
            "resolvedLivePath": normalize_path(resolved_live_path),
        },
        "behaviorMediator": {
        ...
        "pathMap": [
            ...
            },
            {
                ...
                "purpose": "Behavior mediator implementation that writes canonical events into monitor/live/runs/<runId>/.",
            },
            {
                "layer": "run-root",
            ...
            {
                "layer": "viewer",
                "reads": normalize_path(resolved_live_path),
