...
REQUIRED_PATHS = [
    ...
    REPO_ROOT / "guardrail" / "agent_defense" / "bridge.py",
    REPO_ROOT / "guardrail" / "agent_defense" / "trace_merge.py",
    REPO_ROOT / "guardrail" / "agent_defense" / "final_judge.py",
    REPO_ROOT / "dashboard" / "viewer" / "index.html",
    REPO_ROOT / "dashboard" / "viewer" / "app.js",
    REPO_ROOT / "dashboard" / "viewer" / "app.css",
    REPO_ROOT / "dashboard" / "viewer" / "shared.js",
    REPO_ROOT / "tools" / "compat" / "entrypoint.py",
    REPO_ROOT / "tools" / "common" / "trace_common.py",
    REPO_ROOT / "tools" / "runtime" / "start_trace.py",
    REPO_ROOT / "tools" / "runtime" / "setup_runtime.py",
    REPO_ROOT / "tools" / "validate" / "doctor.py",
    REPO_ROOT / "tools" / "validate" / "deployment_doctor.py",
    REPO_ROOT / "tools" / "validate" / "check_portability.py",
    REPO_ROOT / "tools" / "validate" / "run_acceptance.py",
    REPO_ROOT / "tools" / "demo" / "sanitize_showcase_paths.py",
    REPO_ROOT / "tools" / "export" / "export_codetracer_bundle.py",
    REPO_ROOT / "tools" / "diagnosis" / "run_codetracer_diagnosis.py",
    REPO_ROOT / "monitor" / "tests" / "validate" / "test_codetracer_export.py",
    REPO_ROOT / "monitor" / "tests" / "validate" / "test_trace_pipeline.py",
    REPO_ROOT / "tools" / "validate" / "trace_topology.py",
    REPO_ROOT / "tools" / "diagnosis" / "segment_behavior_events.py",
    REPO_ROOT / "monitor" / "vendor" / "runtime-hooks" / "openclaw-behavior-mediator" / "tests" / "behavior-mediator.test.mjs",
]
REMOVED_LEGACY_ENTRYPOINTS = [
    REPO_ROOT / "tools" / "start_trace.py",
    REPO_ROOT / "tools" / "setup_runtime.py",
    REPO_ROOT / "tools" / "export_codetracer_bundle.py",
    REPO_ROOT / "tools" / "run_codetracer_diagnosis.py",
    REPO_ROOT / "tools" / "check_repo.py",
]
REQUIRED_GITIGNORE_PATTERNS = [
    ...
    ".conda*/",
    "monitor/live/**",
    "!monitor/live/.gitkeep",
    "!monitor/live/otel/.gitkeep",
    "monitor/captures/**",
    "!monitor/captures/.gitkeep",
    "monitor/bin/**",
    "!monitor/bin/.gitkeep",
    "monitor/vendor/external/openclaw-observability-plugin/node_modules/",
    "config/*.local.yaml",
    "docs/images/*.png",
...
def check_js_syntax() -> dict[str, Any]:
    files = [
        REPO_ROOT / "dashboard" / "viewer" / "app.js",
        REPO_ROOT / "dashboard" / "viewer" / "shared.js",
        REPO_ROOT / "monitor" / "vendor" / "runtime-hooks" / "openclaw-behavior-mediator" / "index.js",
    ]
    entries: list[dict[str, Any]] = []
    ok = True
    for path in files:
...
def check_gitignore() -> dict[str, Any]:
    ...
    tracked_vendor = run_command(
        ["git", "ls-files", "monitor/vendor/external/openclaw-observability-plugin/node_modules"],
        cwd=REPO_ROOT,
        timeout=30,
        check=False,
    )
...
def run_portability_check() -> dict[str, Any]:
    result = run_command(
        [python_executable(), str(REPO_ROOT / "tools" / "validate" / "check_portability.py")],
        cwd=REPO_ROOT,
        timeout=120,
        check=False,
    )
...
def run_start(host: str, port: int) -> dict[str, Any]:
    result = run_command(
        [
            python_executable(),
            str(REPO_ROOT / "tools" / "runtime" / "start_trace.py"),
            "--viewer-host",
            host,
            "--viewer-port",
            str(port),
...
def run_doctor(host: str, port: int) -> dict[str, Any]:
    result = run_command(
        [
            python_executable(),
            str(REPO_ROOT / "tools" / "validate" / "doctor.py"),
            "--host",
            host,
            "--port",
            str(port),
