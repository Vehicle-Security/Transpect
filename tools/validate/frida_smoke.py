...
from monitor.instrumentation.frida import FridaTraceConfig, FridaTraceManager  # noqa: E402
...
def resolve_node(explicit: str | None) -> str:
    if explicit:
        return explicit
    for candidate in ("/opt/homebrew/monitor/bin/node", shutil.which("node")):
