...
def build_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode in {"core", "hybrid"}:
        ...
        if not report["runs"]["exists"]:
            issues.append("monitor/live/runs is missing")
        elif not report["runs"]["runCount"]:
            warnings.append("monitor/live/runs exists but does not contain any completed or active run manifests yet")
    ...
    if residue["legacyRootLogs"]["fileCount"] > 0:
        warnings.append("legacy log files are present directly under monitor/live/")
    if residue["legacyOpenclawFiles"]["fileCount"] > 0:
        warnings.append("legacy files are present under monitor/live/openclaw/")
