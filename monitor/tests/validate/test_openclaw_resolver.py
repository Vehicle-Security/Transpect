...
class TestOpenClawResolver(unittest.TestCase):
    ...
    @patch("monitor.runtime.agent_scenarios.openclaw_resolver.subprocess.run")
    @patch("monitor.runtime.agent_scenarios.openclaw_resolver.os.access")
    @patch("monitor.runtime.agent_scenarios.openclaw_resolver.Path.exists")
    @patch("shutil.which")
    def test_resolve_success_from_path(self, mock_which: MagicMock, mock_exists: MagicMock, mock_access: MagicMock, mock_run: MagicMock) -> None:
        ...
        mock_which.return_value = "/usr/local/bin/openclaw"
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            self.assertEqual(res.selected_candidate.path, "/usr/local/bin/openclaw")
