...
class RJudgeBatchRunnerTests(unittest.TestCase):
    ...
    def test_agent_trace_command_can_skip_context_judge(self) -> None:
        args = Namespace(
            repo="rjudge",
            timeout=300,
            diagnosis_profile="detailed",
            diagnosis_timeout_seconds=1800,
            diagnosis_cost_limit=3.0,
            skip_diagnosis=False,
            skip_context_judge=True,
            diagnosis_model=None,
        )

        command = run_rjudge_batch.agent_trace_command(args, "data/Application/chatbot.json#37")

        self.assertIn("--skip-context-judge", command)
