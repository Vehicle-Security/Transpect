...
EXPECTED_BEHAVIOR_CONFIG = {
    "outputFile": str((TRACE_LIVE_DIR / "behavior-events.jsonl").resolve()),
...
def count_live_events(path: Path) -> dict[str, Any]:
    result: dict[str, Any] = {
    ...
    if not path.exists():
        ...
        result["schemaError"] = "live file missing"
        return result
    ...
    with path.open("r", encoding="utf-8", errors="replace") as handle:
        for line_number, raw in enumerate(handle, start=1):
            ...
            try:
                ...
                result["schemaOk"] = False
                result["schemaError"] = f"line {line_number}: invalid JSON ({error})"
                return result
            if not isinstance(payload, dict):
                result["schemaOk"] = False
                result["schemaError"] = f"line {line_number}: event must be an object"
                return result
            missing = [field for field in CANONICAL_REQUIRED_FIELDS if payload.get(field) in (None, "")]
            if missing:
                result["schemaOk"] = False
                result["schemaError"] = f"line {line_number}: missing fields {', '.join(missing)}"
                return result
...
def run_gateway_status(timeout_seconds: int) -> dict[str, Any]:
    try:
        payload = get_gateway_status(include_probe=False)
    except Exception as error:  # noqa: BLE001
        return {
            "ok": False,
...
def build_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode in {"core", "hybrid"}:
        ...
        if not report["live"]["exists"]:
            issues.append("live/behavior-events.jsonl is missing")
        elif report["live"]["eventCount"] == 0:
            warnings.append("live/behavior-events.jsonl exists but does not contain events yet")
        if report["live"]["schemaOk"] is False:
            issues.append(f"live file schema validation failed: {report['live']['schemaError']}")
...
def main() -> None:
    ...
    report = {
        ...
        "live": count_live_events(live_path),
        "viewer": {
