...
def _replay_components(root: Path) -> list[dict[str, Any]]:
    return [
        ...
        _exists_component(
            ...
            required=True,
            suggestion="Clone the full repository; requirements.txt is required for Python utilities.",
        ),
        _exists_component(
            "console_app",
            root / "apps" / "console" / "package.json",
            required=True,
            suggestion="Clone the apps/console directory and run npm ci inside it.",
        ),
        _exists_component(
            "console_dependencies",
            root / "apps" / "console" / "node_modules",
            required=True,
            suggestion="Run cd apps/console && npm ci.",
        ),
...
def _live_components(root: Path) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _exists_component(
            "behavior_mediator",
            root / "vendor" / "runtime-hooks" / "openclaw-behavior-mediator" / "index.js",
            required=True,
...
def _rjudge_components(root: Path, *, required: bool) -> list[dict[str, Any]]:
    ...
    return [
        _component(
            ...
            required=required,
            suggestion="Set R_JUDGE_ROOT when running python scripts/runtime/run_task_repo.py --repo rjudge.",
        )
    ]


def _showcase_component(root: Path) -> dict[str, Any]:
    index_path = root / "state" / "showcase" / "index.json"
    if not index_path.exists():
        return _component(
            "showcase_data",
            "failed",
            "state/showcase/index.json missing",
            required=True,
            suggestion="Commit frozen showcase data or generate it with scripts/demo/freeze_showcase_run.py.",
        )
    try:
    ...
    if missing_reports:
        return _component(
            ...
            suggestion="Run python scripts/demo/build_showcase_reports.py.",
        )
    return _component(
        "showcase_data",
        "ok",
