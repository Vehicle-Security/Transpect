...
from app.runtime.openclaw_trace_ingest.schema import OPENCLAW_NATIVE_SOURCES  # noqa: E402
from scripts.common.trace_common import (  # noqa: E402
...
def discover_openclaw_native_sources(run_dir: Path | str | None = None, *, timeout_seconds: int = 4) -> dict[str, Any]:
    ...
    if behavior_plugin["status"] != "available":
        recommendations.append("Install or restore vendor/runtime-hooks/openclaw-behavior-mediator before native source capture.")
    if missing_core:
    ...
    if not resolved_run_dir:
        recommendations.append("Pass --run-dir live/runs/<runId> to inspect native source file coverage for a specific run.")
