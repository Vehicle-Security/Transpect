...
def list_rjudge_tasks(repo: str) -> list[dict[str, Any]]:
    completed = run_command(
        [
            sys.executable,
            "scripts/runtime/run_task_repo.py",
            "--repo",
            repo,
            "--mode",
            "list-tasks",
...
def run_runtime_setup(open_viewer: bool) -> dict[str, Any]:
    command = [sys.executable, "scripts/runtime/start_trace.py"]
    if not open_viewer:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--batch-name", help="Stable report name under live/reports without .json.")
